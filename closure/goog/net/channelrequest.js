...
//  This file depends on goog.net.BrowserChannel and vice versa (a
...
goog.net.ChannelRequest.prototype.decodeNextChunks_ = function(readyState,
  ...
  var jsonParser = new goog.json.EvalJsonProcessor();
  while (!this.cancelled_ &&
         this.xmlHttpChunkStart_ < responseText.length) {
    // TODO(jcai):
    var chunkText = this.getNextChunk_(responseText);
    if (goog.isString(chunkText) && goog.string.startsWith(chunkText, "{\"yiqixie\"")) {
      var chunkTextObj = jsonParser.parse(chunkText);
      chunkText = goog.crypt.obfuscation.deobfuscate(chunkTextObj['yiqixie']);
    }
...
goog.net.ChannelRequest.prototype.getNextChunk_ = function(responseText) {
  ...
  //console.log('return chunk text: ' + chunkText);
  return chunkText;
};
...
/**
 * Called when the request has actually timed out. Will cleanup and notification the

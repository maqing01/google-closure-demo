...
// ws日志，只能放在这里的原因是：这个ws实现太shit，应有的错误没有暴露给上层
goog.require('office.net.WebsocketLogger');
...
goog.net.WebSocket.prototype.onClose_ = function(event) {
    office.net.WebsocketLogger.onClose(event);
    goog.log.info(this.logger_, 'The WebSocket on ' + this.url_ + ' closed.');

    // Firing this event allows handlers to query the URL.
    this.dispatchEvent(goog.net.WebSocket.EventType.CLOSED,event);
...
goog.net.WebSocket.prototype.onError_ = function(event) {
    office.net.WebsocketLogger.onError(event);

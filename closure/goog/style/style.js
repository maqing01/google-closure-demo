...
goog.style.getCascadedStyle = function(element, style) {
  //  This should be documented to return null. #fixTypes
...
goog.style.getPageOffset = function(el) {
  ...
  //  Update the jsdoc in a way that doesn't break the universe.
  ...
  //  Should we check if the node is disconnected and in that case
  //            return (0,0)?
  ...
  // Gecko prior to 1.9.
    ...
    //  Fix the off-by-one error when window is scrolled down
...
goog.style.setOpacity = function(el, alpha) {
  ...
  if ('opacity' in style) {
    ...
    //  Overwriting the filter might have undesired side effects.
    if (alpha === '') {
      style.filter = '';
    } else {
...
goog.style.setUnselectable = function(el, unselectable, opt_noRecurse) {
  //  Do we need all of TR_DomUtil.makeUnselectable() in Closure?
